...
namespace Community.PowerToys.Run.Plugin.Everything
{
    internal class ContextMenuLoader : IContextMenu
    {
        ...
        public List<ContextMenuResult> LoadContextMenus(Result selectedResult)
        {
            ...
            {
                // Test to check if File can be Run as admin, if yes, we add a 'run as admin'
                // context menu item
                ...
                {
                    contextMenus.Add(CreateOpenFolderResult(record));
                }
        ...
        private static ContextMenuResult CreateRunAsUserContextMenu(SearchResult record)
        {
            ...
            {
                PluginName = Assembly.GetExecutingAssembly().GetName().Name,
                Title = Properties.Resources.Community_plugin_everything_run_as_user,
                Glyph = "\xE7EE",
                FontFamily = "Segoe MDL2 Assets",
                ...
                Action = _ =>
                {
                    try
                    {
                        Task.Run(() => Helper.RunAsUser(record.FullPath));
                        return true;
                    }
                    catch (System.Exception e)
                    {
        ...
        private ContextMenuResult CreateOpenFolderResult(SearchResult record)
        {
            return new ContextMenuResult
            {
                PluginName = Assembly.GetExecutingAssembly().GetName().Name,
                ...
                Action = _ =>
                {
                    if (!Helper.OpenInShell("explorer.exe", $"{record.FullPath}"))
                    {
                        var message = $"{Properties.Resources.Community_plugin_everything_folder_open_failed} {record.FullPath}";
                        _context.API.ShowMsg(message);
                        return false;
